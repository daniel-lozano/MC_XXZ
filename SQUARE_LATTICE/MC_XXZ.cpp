...
//    double Tc=2.0/log(1+sqrt(2))*J; //
    ...
    for(int i=0;i<H;i++){
        for(int j=0;j<L;j++){
//            cout<<"i="<<i<< " j="<< j<< " Pos="<< i*L+j <<endl;
            neighbours[i*L+j][0]=i*L+(j+1)%L; //Right neighbour
//            cout<<neighbours[i*L+j][0]<<endl;
            
            neighbours[i*L+j][1]=((i-1+H)%H)*L+j; //Upwards neighbour
//            if(i==0){neighbours[i*L+j][1]=(H-1)*L+j; }
//            cout<<neighbours[i*L+j][1]<<endl;
            
            neighbours[i*L+j][2]=i*L+(j-1+L)%L; //Left neighbour
//            if(j==0){neighbours[i*L+j][2]=i*L+ L-1; }
//            cout<<neighbours[i*L+j][2]<<endl;
            
            neighbours[i*L+j][3]=((i+1)%H)*L+j; //Downwards neighbour
//            cout<<neighbours[i*L+j][3]<<endl;
        }
    }
